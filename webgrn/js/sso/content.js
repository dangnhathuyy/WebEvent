...
function showContent(content, mode, showBanner) {
  ...
  if (getRequestParam('display') != 'embedded') {
    ...
    if (mode == 'pc') {
      var langWrapper = $('<div/>')
        .addClass('langWrapper')
        .addClass('fr')
        .appendTo(header)
      var langSelect = $('<select/>').addClass('lang').appendTo(langWrapper)
      for (i in LOCALE_LIST) {
        var option = $('<option/>', {
          value: getLocale(LOCALE_LIST[i].code, LOCALE_LIST[i].region),
          text: LOCALE_LIST[i].lname,
        }).appendTo(langSelect)
      }
    ...
    if (!SETTINGS.iframe) {
      $('<img/>', {
        src: './images/img_garena_logo.png',
        height: 35,
      }).appendTo(link)
    } else {
  ...
  if (SETTINGS['gas_download'] && showBanner) {
    ...
    $('<img/>', {
      src: SETTINGS['static_root'] + 'images/garena_banner.png',
    }).appendTo(gas)
    $('<span/>').addClass('bannerText').text(_('gas_intro')).appendTo(gas)
    var btnDownload = $('<a/>', { href: 'javascript:;' })
      .addClass('download')
      .appendTo(gas)
...
function showMobileRegisterAlert(msg) {
  ...
  var btnOK = $('<a/>', { href: 'javascript:;', class: 'btn-ok' })
    .text(_('btn_ok'))
    .appendTo(container)
...
function showDialog(content, id, title, onCloseCallback) {
  removeDialog()
  var container = $('<div/>')
    .addClass('sso_dialog_container')
    .css('display', 'none')
  var background = $('<div/>')
    .addClass('sso_dialog_background')
    .appendTo(container)
  ...
  var btnClose = $('<div/>', { title: _('dialog_close_tip') })
    .html('&times;')
    .addClass('sso_dialog_close')
    .appendTo(dialog)
