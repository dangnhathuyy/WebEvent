;(function () {
  ...
  function appendPlatformsPanel(container) {
    ...
    if (
      SETTINGS.platform != PLATFORM_GARENA &&
      SETTINGS.show_platforms & (1 << (PLATFORM_GARENA - 1))
    ) {
      var linkGarena = $('<a/>', { href: 'javascript:;' })
        .addClass('icon-garena')
        .appendTo(links)
    ...
    if (
      SETTINGS.mode == 'oauth_login' &&
      SETTINGS.platform != PLATFORM_FACEBOOK &&
      SETTINGS.show_platforms & (1 << (PLATFORM_FACEBOOK - 1))
    ) {
      var linkFacebook = $('<a/>', { href: 'javascript:;' })
    ...
    if (
      SETTINGS.mode == 'oauth_login' &&
      SETTINGS.platform != PLATFORM_LINE &&
      SETTINGS.show_platforms & (1 << (PLATFORM_LINE - 1))
    ) {
      var linkLine = $('<a/>', { href: 'javascript:;' })
        .addClass('icon-line')
        .appendTo(links)
    ...
    if (
      SETTINGS.mode == 'oauth_login' &&
      SETTINGS.platform != PLATFORM_VK &&
      SETTINGS.show_platforms & (1 << (PLATFORM_VK - 1))
    ) {
      var linkVk = $('<a/>', { href: 'javascript:;' })
        .addClass('icon-vk')
        .appendTo(links)
    ...
    if (
      SETTINGS.mode == 'oauth_login' &&
      SETTINGS.platform != PLATFORM_GOOGLE &&
      SETTINGS.show_platforms & (1 << (PLATFORM_GOOGLE - 1))
    ) {
      var linkGoogle = $('<a/>', { href: 'javascript:;' })
        .addClass('icon-google')
        .appendTo(links)
      linkGoogle.click(loginGoogle)
    }
    if (
      SETTINGS.mode == 'oauth_login' &&
      SETTINGS.platform != PLATFORM_HUAWEI &&
      SETTINGS.show_platforms & (1 << (PLATFORM_HUAWEI - 1))
    ) {
      var linkHuawei = $('<a/>', { href: 'javascript:;' })
        .addClass('icon-huawei')
        .appendTo(links)
      linkHuawei.click(loginHuawei)
    }
    if (
      SETTINGS.mode == 'oauth_login' &&
      SETTINGS.platform != PLATFORM_APPLE &&
      SETTINGS.show_platforms & (1 << (PLATFORM_APPLE - 1))
    ) {
      var linkApple = $('<a/>', { href: 'javascript:;' })
        .addClass('icon-apple')
        .appendTo(links)
    ...
    if (
      SETTINGS.mode == 'oauth_login' &&
      SETTINGS.platform != PLATFORM_TWITTER &&
      SETTINGS.show_platforms & (1 << (PLATFORM_TWITTER - 1))
    ) {
      var linkTwitter = $('<a/>', { href: 'javascript:;' })
  ...
  function preloginCallback(data, status_code) {
    ...
    if ('error' in data) {
      if (data['error'] == 'error_require_captcha') {
        ...
        renderRecaptcha('sso_recaptcha_wrap', function (token) {
          login_recaptcha_token = token
        })
        ...
        showShopeeCaptchaDialog(login_captcha_dialog, function (token) {
          login_shopee_captcha_token = token
          hideCaptchaDialog(login_captcha_dialog)
        })
    ...
    var passwordKey = CryptoJS.SHA256(
      CryptoJS.SHA256(passwordMd5 + data.v1) + data.v2
    )
    var encryptedPassword = CryptoJS.AES.encrypt(passwordMd5, passwordKey, {
  ...
  function loginCallback(data) {
    if ('error' in data) {
      if (data['error'] == 'error_require_captcha') {
        ...
        var timeout = 7
        showError(
          _('login_error_security_ban', { second: timeout }),
          $('#line-btn')
        )
  ...
  function showRegisterCaptchaPage() {
    ...
    $('<h2/>')
      .addClass('title')
      .text(_('register_captcha_header'))
      .appendTo(panelRegisterCaptcha)

    var formRegisterCaptcha = $('<form/>', { id: 'form-register' })
      .addClass('loginForm')
      .appendTo(panelRegisterCaptcha)
      .submit(false)
    var captchaWrapper = $('<div/>', {
      class: 'line clearfix',
      id: 'sso_captcha_wrap',
    }).appendTo(formRegisterCaptcha)
    var rowCaptcha1 = $('<div/>', {
      id: 'line-captcha',
      class: 'sso_captcha',
    }).appendTo(captchaWrapper)
    var inputCaptcha = $('<input/>', {
      id: 'input-captcha',
      name: 'captcha',
      type: 'text',
      placeholder: _('register_form_captcha_prompt'),
      autocorrect: 'off',
      autocapitalize: 'off',
    }).appendTo(rowCaptcha1)
    var rowCaptcha2 = $('<div/>', {
      id: 'sso_captcha_image',
      class: 'clearfix sso_captcha',
    }).appendTo(captchaWrapper)
    captcha_key = uuid().replace(/-/g, '')
    $('<img/>', {
      src: SETTINGS.test
        ? CAPTCHA_SERVICE_TEST
        : CAPTCHA_SERVICE + '?key=' + captcha_key,
    }).appendTo($('<span/>', { class: 'code fl' }).appendTo(rowCaptcha2))
    var refresh_captcha = $('<a/>', {
      href: 'javascript:;',
      class: 'refresh fl',
    })
      .text(_('refresh_captcha'))
      .appendTo(rowCaptcha2)
    refresh_captcha.click(refreshCaptcha)
    refreshCaptcha()

    var lineVerify = $('<div/>', { id: 'line-verify' }).appendTo(
      panelRegisterCaptcha
    )
    var btnVerify = $('<a/>', { href: 'javascript:;', class: 'btn btn-login' })
      .text(_('btn_captcha_verify'))
      .appendTo(lineVerify)
    ...
    btnVerify.click(function () {
      var request = {
        ...
        mobile_no:
          mobile_register_request['mobile_no1'] +
          mobile_register_request['mobile_no2'],
      }
  ...
  function showRegisterVerifyMobilePage() {
    ...
    $('<h2/>')
      .addClass('title')
      .text(_('register_verify_mobile_header'))
      .appendTo(panelRegisterVerifyMobile)
    var issues = $('<div/>', {
      id: 'line-issues',
      class: 'issues center',
    }).appendTo(panelRegisterVerifyMobile)
    ...
    var resendLine = $('<div/>', {
      id: 'line-resend',
      class: 'resendLine clearfix',
    }).appendTo(panelRegisterVerifyMobile)
    var verification_code = $('<input/>', {
      type: 'tel',
      placeholder: _('register_verify_mobile_prompt'),
      class: 'fl',
    }).appendTo(resendLine)
    var btnResend = $('<a/>', {
      href: 'javascript:;',
      class: 'btn btn-grey btn-resend fr',
    })
      .text(_('btn_register_verify_resend'))
      .appendTo(resendLine)

    var lineVerify = $('<div/>', { id: 'line-verify' }).appendTo(
      panelRegisterVerifyMobile
    )
    var btnVerify = $('<a/>', { href: 'javascript:;', class: 'btn btn-login' })
      .text(_('btn_register_verify'))
      .appendTo(lineVerify)
    ...
    function resendCooldown() {
      ...
      btnResend.text(
        _('btn_register_verify_resend_otp_timer', { second: leftSeconds })
      )
  ...
  function showGrantPage(mode) {
    ...
    var btnDecline = $('<a/>', {
      id: 'sso_auth_from_decl_button',
      type: 'submit',
      text: _('grant_form_button_decline'),
    })
      .addClass('btn-cancel')
      .appendTo(btn_group)
    var btnAuthorize = $('<a/>', {
      id: 'sso_auth_from_auth_button',
      type: 'submit',
      text: _('grant_form_button_confirm'),
    })
    ...
    var linkChangeAccount = $('<a/>', {
      id: 'auth_link_change_account',
      text: _('grant_link_change_account_text'),
      href: 'javascript:;',
    }).appendTo(panelLink)
  ...
  function showLoginPage(mode) {
    ...
    var formLogin = $('<form/>', { id: 'login-form' })
      .addClass('loginForm')
      .appendTo(panelLogin)
    ...
    var selectCountry = $('<select/>', {
      id: 'sso_login_form_select_country',
      name: 'select_country',
    })
      .addClass('country')
      .appendTo(lineCountry)
    for (i in COUNTRY_LIST) {
      var option = $('<option/>', {
        value: COUNTRY_LIST[i].code,
        'data-country-code': COUNTRY_LIST[i].pcode,
      })
    ...
    var lineAccount = $('<div/>', { id: 'line-account' })
      .addClass('line')
      .appendTo(formLogin)
    var txtCountryCode = $('<span/>', {
      id: 'sso_login_form_country_code',
      style: 'display: none;',
    })
    ...
    var txtAccount = $('<input/>', {
      ...
      name: 'username',
      type: 'text',
      placeholder: _('login_form_account_prompt'),
      autocorrect: 'off',
      autocapitalize: 'off',
    }).appendTo(lineAccount)
    if (
    ...
    var txtPassword = $('<input/>', {
      ...
      name: 'password',
      type: 'password',
      placeholder: _('login_form_password_prompt'),
    }).appendTo(
      $('<div/>', { id: 'line-password' }).addClass('line').appendTo(formLogin)
    )
    ...
    $('<input/>', {
      ...
      name: 'captcha',
      type: 'text',
      placeholder: _('login_form_captcha_prompt'),
      autocorrect: 'off',
      autocapitalize: 'off',
    }).appendTo(rowCaptcha1)
    var rowCaptcha2 = $('<div/>', {
    ...
    $('<img/>').appendTo(
      $('<span/>', { class: 'code fl' }).appendTo(rowCaptcha2)
    )
    var refresh_captcha = $('<a/>', {
      href: 'javascript:;',
      class: 'refresh fl',
    ...
    $('<div/>', { class: 'line clearfix', id: 'sso_recaptcha_wrap' }).appendTo(
      formLogin
    )
    ...
    var btnLogin = $('<input/>', {
      ...
      name: 'login',
      type: 'submit',
      value: _('login_form_button_login'),
      onclick: 'javascript:return false;',
    })
      .addClass('btn')
      .appendTo(
        $('<div/>', { id: 'line-btn', class: 'line btnLine' }).appendTo(
          formLogin
        )
      )

    var dividerOr = $('<span/>')
      .text(_('sso_login_divider_text'))
      .appendTo($('<div/>', { class: 'divider' }).appendTo(formLogin))

    var btnRegister = $('<input/>', {
      id: 'sso_login_link_register',
      name: 'register',
      type: 'button',
      value: _('sso_login_link_register_text'),
      onclick: 'javascript:return false;',
    })
      .addClass('btn')
      .addClass('black')
    ...
    var linkForgetPassword = $('<a/>', {
      id: 'sso_login_link_forget_password',
      text: _('sso_login_link_forget_password_text'),
      href: 'javascript:;',
    })
      .addClass('sec')
      .appendTo(panelLink)
  ...
  function loginVk() {
    ...
    params.display = 'page'
    params.redirect_uri = updateURLParameter(
  ...
  function loginApple() {
    ...
    for (var key in requestParams) {
      if (key == 'redirect_uri')
        requestParams[key] = encodeURIComponent(requestParams[key])
  ...
  function loginTwitter() {
    ...
    params.callback_uri =
      window.location.href.split('?')[0] +
      '?' +
      encodeURIComponent(sParams.substr(1))

    requestJson(
      SSO_URL_OAUTH_TWITTER_REQUEST_TOKEN,
      params,
      function (data) {
        params = {}
        if ('error' in data) {
  ...
  function exchangeVkToken(access_token) {
    var redirect_uri = getRequestParam('redirect_uri')
    requestJson(
      SSO_URL_OAUTH_TOKEN_VK_EXCHANGE,
      {
  ...
  function exchangeLineToken(token) {
    var redirect_uri = getRequestParam('redirect_uri')
    var line_redirect_uri = updateURLParameter(
      window.location.href.split('#')[0],
      'code',
      ''
    )
    ...
    requestJson(
      SSO_URL_OAUTH_TOKEN_LINE_EXCHANGE,
      {
        response_type: SETTINGS.response_type,
  ...
  function exchangeGoogleToken(google_auth_code) {
    var redirect_uri = getRequestParam('redirect_uri')
    requestJson(
      SSO_URL_OAUTH_TOKEN_GOOGLE_EXCHANGE,
      {
        response_type: SETTINGS.response_type,
  ...
  function exchangeHuaweiToken(auth_code) {
    var redirect_uri = getRequestParam('redirect_uri')
    requestJson(
      SSO_URL_OAUTH_TOKEN_HUAWEI_EXCHANGE,
      {
        response_type: SETTINGS.response_type,
        hw_auth_code: auth_code,
        client_id: SETTINGS['app_id'],
        huawei_redirect_uri: updateURLParameter(
          window.location.href.split('#')[0],
          'authorization_code',
          ''
        ),
  ...
  function exchangeAppleToken(apple_auth_code) {
    var redirect_uri = getRequestParam('redirect_uri')
    requestJson(
      SSO_URL_OAUTH_TOKEN_APPLE_EXCHANGE_WEB,
      {
        response_type: SETTINGS.response_type,
  ...
  function exchangeTwitterToken(twitter_oauth_token, oauth_verifier) {
    var redirect_uri = getRequestParam('redirect_uri')
    requestJson(
      SSO_URL_OAUTH_TOKEN_TWITTER_EXCHANGE,
      {
        response_type: SETTINGS.response_type,
  ...
  function exchangeFacebookToken(access_token) {
    var redirect_uri = getRequestParam('redirect_uri')
    requestJson(
      SSO_URL_OAUTH_TOKEN_FACEBOOK_EXCHANGE,
      {
        response_type: SETTINGS.response_type,
